...
\begin{tabularx}{\textwidth}{L M}
    ...
    \grigioriga
    Molle in parallelo & k_{eq} = k_1 + k_2 \\[1.5ex]
    \grigioriga
    Molle in serie & \frac{1}{k_{eq}} = \frac{1}{k_1} + \frac{1}{k_2} \\[1.5ex]
    \grigioriga
...
\begin{tabularx}{\textwidth}{L M}
    ...
    \grigioriga
    Costante di proporzionalità & K = \frac{1}{4 \pi \epsilon_0 [\epsilon_r]} \text{ con $\epsilon_r$ solo non nel vuoto} \\
    \grigioriga
    Coulomb, cariche puntiformi & \vec{F_{ab}} = \frac{1}{4 \pi \epsilon_0} \frac{q_a q_b}{r^2} \hat{r}_{ab} \text{ , } \vec{E} = \frac{1}{4 \pi \epsilon_0} \frac{q}{r^2} \hat{r} \\
    \grigioriga
    ...
    \grigioriga
    Potenziale per carica & V = \frac{1}{4 \pi \epsilon_0} \frac{q}{r} \\
    \grigioriga
    Energia potenziale elettrica & U = V q \\
    \grigioriga
    Capacità & C = \frac{Q}{V} \\
    \grigioriga
    Condensatore piano & C = \frac{[\epsilon_r] \epsilon_0 A}{d} \\
    \grigioriga
    Condensatori in serie & \frac{1}{C} = \sum_i \frac{1}{  C_i} \\    
    \grigioriga
    Condensatori in parallelo & C = \sum_i C_i \\
    \grigioriga
    Energia potenziale condensatore & U = \frac{Q^2}{2C} = \frac{1}{2} C V^2 = \frac{QV}{2} \\   
    \grigioriga
    Densità di energia & u = \frac{U}{A d} = \frac{1}{2} \epsilon_0 E^2 \\
    \grigioriga
    Intensità di corrente & I = \frac{dQ}{dt} = n S v_d |q| \text{, con $n$ densità portatori, $S$ sezione filo, $v_d$ velocità deriva} \\
    \grigioriga
    Densità volumica & n = \frac{dN}{dV} \text{, con $N$ numero di particelle e $V$ volume} \\
    \grigioriga
    Densità corrente & J = \frac{I}{S} = n q v_d \\
    \grigioriga
    Legge di Ohm & V = R I \\
    \grigioriga
    ...
    \grigioriga
    Drude velocità deriva & \vec{v_d} = -\frac{e E}{m} \tau \text{ con $\tau$ tempo medio tra urti} \\
    \grigioriga
    Drude resistività & \rho = \frac{m}{n e^2 \tau} \\
    \grigioriga
    Resistenze in serie & R_{tot} = R_1 + R_2 \\
    \grigioriga
    Resistenze in parallelo & \frac{1}{R_{tot}} = \frac{1}{R_1} + \frac{1}{R_2} \\
    \grigioriga
    Legge di Joule & P = V I = R I^2 = \frac{V^2}{R} \\
    \grigioriga
    Energia dissipata in resistenza & \Delta U = -V \Delta Q \\
    \grigioriga
    1° Legge Kirchoff & I_{in_1} + I_{in_2} = I_{out_1} + I_{out_2} \\
    \grigioriga
    2° Legge Kirchoff & V - R_1 I - R_2 I - R_3 I = 0 \\
    \grigioriga
    Carica condensatore & q(t) = C \Delta V ( 1 - e ^ {- \frac{t}{RC}}) \\
    \bottomrule
\end{tabularx}
